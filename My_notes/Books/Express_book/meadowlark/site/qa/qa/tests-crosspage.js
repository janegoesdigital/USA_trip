...
suite('Cross-Page Tests', function(){
 ...
 test('requesting a group rate from the oregon coast tour page should ' + 'populate the referrer field', function(done){
     var referrer = 'http://localhost:300/tours/oregon-coast';
     browser.visit(referrer, function(){
      browser.clickLink('.requestGroupRate', function(){
       assert(browser.field('referrer').value === referrer);
       done();
 ...
 test('visiting the "request group rate" page directrly should result' + 'in an empty referrer field', function(done){
  browser.vist('http://localhost:3000/tours/request-group-rate', function(){
